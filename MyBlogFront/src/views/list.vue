<template>
	<div class="list-box">
		...
		<!-- 博客列表元素 -->
		<div v-for="(item, index) in dataList" :key="index" class="card" :style="`${color};${cardBg}`">
			<div @click="articleDetail(item)">
				<div class="head">
					<h3>{{ item.title }}</h3>
					<div>{{ item.type == 'blog' ? '博客' : '心情' }}</div>
				</div>
		...
		<el-empty v-if="dataList.length == 0" description="先去别的地方看看吧！"></el-empty>
		<!-- 分页 -->
		<el-pagination
			background
...
export default {
	...
	methods: {
		...
		articleDetail(item) {
			try {
				this.$router.push(`/article/${item.classification ? 'blog' : 'jotting'}/${item._id}`);
			} catch (error) {
